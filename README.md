...
- Claude AI 智能摘要和分类
- 支持相对日期查询（昨天、前天等）
- **智能日期回退**：当目标日期无资讯时，自动查找最近 7 天内的资讯
...
A: 系统会自动启用**智能日期回退**功能：
- 如果目标日期（如昨天）没有资讯，会自动查找前 1-7 天的资讯
- 找到资讯后会在控制台显示实际使用的日期
- 如果 7 天内都没有资讯，才会提示无数据并列出可用日期范围

例如：请求 2026-02-03 的资讯，但 RSS 最新只到 2026-01-30，系统会自动回退到 2026-01-30 并成功获取资讯。
